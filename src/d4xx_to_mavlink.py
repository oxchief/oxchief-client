...
# This script is a modified version of the original script by Thien Nguyen.

# Credit: https://github.com/thien94/vision_to_mavros/blob/master/scripts/d4xx_to_mavlink.py
# Author: Thien Nguyen
# License: GPL-3.0
...
DEVICES_FILENAME = Constants.DEVICES_FILENAME_REALSENSE

def list_attached_devices() -> list[list[str,str]]:
    """
    ...
    """
    devices = open(DEVICES_FILENAME, "r")
    lines = devices.readlines()
    device_list = [[s.rstrip() for s in i.split(" - ", 1)] for i in lines]
    return device_list

def realsense_serial_port_name(config:config.Config) -> str:
    """
    ...
    for i in attached_devices:
        device_name = i[1]
        if device_name.find(config.ardupilot_realsense_serial_name_substring) != -1:
            return i[0]
    return False

print(f"ardupilot_realsense_serial_name_substring: {config_obj.ardupilot_realsense_serial_name_substring}")

print(f"realsense_serial_port_name: {realsense_serial_port_name(config_obj)}")
...
COLOR_HEIGHT = 480
FPS          = 30
DEPTH_RANGE_M = [0.1, 3.0]       # Replace with your sensor's specifics, in meter

obstacle_line_height_ratio = 0.16  # [0-1]: 0-Top, 1-Bottom. The height of the horizontal line to find distance to obstacle.
...
PRESET_FILE  = "/usr/src/app/cfg/d4xx-default.json"
#PRESET_FILE = "/home/pi/src/oxchief/vision_to_mavros/cfg/d4xx-default.json"
...
# /dev/ttyAMA1,921600
connection_string_default = realsense_serial_port_name(config_obj)
connection_baudrate_default = config_obj.ardupilot_baud
