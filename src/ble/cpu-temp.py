...
# constants
# Custom service uuid
CPU_TMP_SRVC = '12341042-1234-1234-1234-123456789aaa'
...
def read_value():
    """
    Example read callback. Value returned needs be to a list of bytes/integers
    ...
    print("little endian signed bytes (negative):")
    littleEndianSignedBytes = list(int(-cpu_value).to_bytes(2, byteorder='little', signed=True))

    cpu_value = random.randrange(3200, 5310, 10) / 100
    #print(cpu_value)
    # return list(int(cpu_value * -100).to_bytes(2,
    #                                           byteorder='little', signed=True))
    string_val = f"It's actually this hot: {cpu_value}"
    print(string_val)
    return bytearray(string_val.encode())
    
    cpu_value = random.randrange(3200, 5310, 10) * -1
    print(cpu_value)
    littleEndianSignedBytes = list(int(cpu_value).to_bytes(2, byteorder='little', signed=True))
    
    print(littleEndianSignedBytes)
    print("little endian unsigned bytes:")
    littleEndianUnSignedBytes = list(int(cpu_value).to_bytes(2, byteorder='little', signed=False))
    print(littleEndianUnSignedBytes) 
    return littleEndianSignedBytes
    #littleEndianUnSignedBytes = list(int(cpu_value).to_bytes(2, byteorder='little', signed=False))
    #print(littleEndianUnSignedBytes)
...
def notify_callback(notifying, characteristic):
    ...
    which calls the update callback every _ seconds
